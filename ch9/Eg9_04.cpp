...
int main()
{
  ...
  std::array array_data { 3.5, 5.0, 6.0, -1.2, 8.7, 6.4 }; // Throwing in std::array for good measure
  std::vector<std::string> names { "Charles Dickens", "Emily Bronte", 
                                  "Jane Austen", "Henry James", "Arthur Miller"};
  std::println("The largest of the array is {}", largest(array));
  std::println("The largest of the numbers is {}", largest(numbers));
  std::println("The largest of the array_data is (also) {}", largest(array_data));
  std::println("The largest of then names is {}", largest(names));
