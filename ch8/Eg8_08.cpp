// Ex8_08.cpp - Using a reference parameter
...
void find_words(vector<string>& words, const string& str, const string& spearators);
...
int main()
{
  ...
  const std::string spearators {" ,;:.\"!?'\n"};          // Word delemiters
  std::vector<std::string> words;                         // Words found
  find_words(words, text, spearators);
  list_words(words);
}
