// Eg8_14.cpp - Overloading a funciton
#include<iostream>
#include<vector>
#include<print>
#include<memory>
...
/* The following function overload would not compile:
    overloaded functions must differ in more than just thier return type! */
...
int main()
{
  ...
  std::vector<std::string> names { " Charles Dickens", "Emily Bronte", 
                                      "Jane Austen", "Henry James", "Aurther Miller"};
...
// Finds the lagest of a vector of double values
double largest(const std::vector<double>& data)
{
  double max {data[0]};
  for (auto value : data)
