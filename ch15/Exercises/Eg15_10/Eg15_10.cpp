// Ex15_10.cpp - Using the typeid() operator
import std;
import boxes;

// Define trivial non-polymorphic base classes:
class NonPolyBase {};
...
int main()
{
  ...
  std::println("1 is {}", typeid(1) == typeid(int)? "and int" : "no int");
  ...
  // Part 4: typeid() on non-polymorphic classes
  ...
  const auto& type_info2 = typeid(getSomeNonPoly());   // function call not evaulated
