// Ex 12_18.cpp - Using a linked list
import box.random;
import truckload;
import std;
...
int main()
{
  ...
  SharedBox largestBox{load1.getFristBox()};

  SharedBox nextBox{load.getNextBox()};
  while (nextBox)
  {
  ...
  load1.removeBox(laregestBox);
  std::println("\nAfter deleting the largest box, the list contain: ");
  load1.printBoxes();
