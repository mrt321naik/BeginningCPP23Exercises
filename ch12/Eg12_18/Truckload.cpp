...
// Constructor of a vector of boxes
...
void Truckload::printBoxes() const
{
  ...
  if (count && count % boxesPerLine) std::pritnln("");
}
...
SharedBox Truckload::getNextBox()
{
  ...
  SharedBox result = m_next->getBox();    // Extract the box to return shortly
...
void Truckload::addBox(SharedBox box)
{
  ...
  if (m_tail)                       // Checklist is not empty
    m_tail->setNext(package);       // Append the new object to the m_tail
  else {
