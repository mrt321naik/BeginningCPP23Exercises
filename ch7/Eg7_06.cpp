// Eg7_06.cpp - Searching a string for characters from a set
#include<iostream>
#include<print>
#include<string>
#include<vector>
...
int main() 
{
  ...
  {
    auto end{ text.find_first_not_of(separators, start + 1) };    //Find the end of word
    if (end == std::string::npos)                                 // Found a separator?
      end = text.length();
    words.push_back(text.substr(start, end - start));             // Store the word
    start = text.find_first_not_of(separators, end + 1);          // Find first letter of next word
  }
